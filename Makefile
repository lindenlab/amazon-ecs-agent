...
.PHONY: all gobuild static docker release certs gotest test-in-docker clean netkitten test-registry run-functional-tests gremlin benchmark-test gogenerate run-integ-tests image-cleanup-test-images
...
build-in-docker:
	...
	@docker run --net=none -e TARGET_OS="${TARGET_OS}" \
	  ...
	  "registry.docker/amazon/amazon-ecs-agent-build:make"
...
docker-release:
	...
	@docker run --net=none -e TARGET_OS="${TARGET_OS}" -v "$(shell pwd)/out:/out" \
	  -v "$(shell pwd):/src/amazon-ecs-agent" "registry.docker/amazon/amazon-ecs-agent-cleanbuild:make"
...
clean:
	...
	-$(MAKE) -C misc/netkitten $(MFLAGS) clean || true
	-$(MAKE) -C misc/volumes-test $(MFLAGS) clean || true
	-$(MAKE) -C misc/gremlin $(MFLAGS) clean || true
	-$(MAKE) -C misc/testnnp $(MFLAGS) clean || true
	-$(MAKE) -C misc/image-cleanup-test-images $(MFLAGS) clean || true
